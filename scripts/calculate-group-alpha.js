...
async function main() {
  try {
    console.log('開始計算第1-5組的 K-Alpha...\n');
    ...
    for (const project of projects) {
      console.log(`========== 第 ${project.id} 組: ${project.name} ==========`);

      // 取得該專案的所有標註資料 (只取初次標注: version = 1 或 reannotation_round = 0)
      const { rows: annotations } = await sql`
        ...
        WHERE sd.project_id = ${project.id}
          AND (a.version = 1 OR a.reannotation_round IS NULL OR a.reannotation_round = 0)
        ORDER BY a.source_data_id, u.username;
      `;

      if (annotations.length === 0) {
        console.log('❌ 此專案沒有初次標註資料\n');
        continue;
      }
      ...
      console.log(`   標註者: ${annotators.join(', ')}`);
      console.log(`   項目數: ${sourceDataList.length}`);
      ...
      for (const item of sourceDataList) {
        ...
        if (completedAnnotators.length < annotators.length) {
          allComplete = false;
          incompleteItems.push(item.source_data_id);
        }
      }

      if (!allComplete) {
        console.log(`   ⚠️  有 ${incompleteItems.length} 個項目沒有被所有標註者完成標註`);
        console.log(`   跳過此組的K-Alpha計算\n`);
        continue;
      }

      console.log(`   ✓ 所有項目已完成標註\n`);
      ...
      for (const task of tasks) {
        ...
        console.log(`      • ${taskNames[task]}: ${isNaN(alpha) ? 0 : alpha.toFixed(4)} (${quality})`);
      }
